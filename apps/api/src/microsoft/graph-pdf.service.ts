 ...
 *
 * תהליך: העלאת הקובץ ל-OneDrive הייעודי (קובץ זמני) → הורדה חזרה בפורמט PDF → מחיקת הקובץ הזמני.
 * ההמרה מתבצעת *תמיד* דרך חשבון ה-OneDrive הנפרד המחובר (getPdfConverterAccessToken), ולעולם לא
 * דרך חשבון ה-Outlook/ההתחברות של המשתמש הקורא — כך שכל המשתמשים ממירים דרך אותו חשבון OneDrive.
 * דורש הרשאת Files.ReadWrite (delegated) על אותו חשבון. אם אין חשבון OneDrive מחובר — ההמרה נכשלת
 * בשגיאה ברורה (בלי נפילה-חזרה לחשבון הקורא).
 */
@Injectable()
export class GraphPdfService {
  ...
  async fileToPdf(userId: string, file: Buffer, fileName: string, mimeType?: string): Promise<Buffer> {
    // ההמרה ל-PDF תמיד עוברת דרך חשבון ה-OneDrive *הייעודי* המחובר (החשבון הנפרד — למשל של אורי),
    // ולעולם לא דרך חשבון ה-Outlook/ההתחברות של המשתמש הקורא. getPdfConverterAccessToken מאתר
    // את החשבון המחובר ומנפיק לו טוקן, ללא תלות ב-userId הקורא. אם אין חשבון OneDrive מחובר —
    // הוא זורק שגיאה ברורה (בלי נפילה-חזרה לחשבון הקורא). ה-userId נשמר לצורך תאימות/לוגים בלבד.
    const { token } = await this.auth.getPdfConverterAccessToken();
    ...
    try {
      ...
      if (!upRes.ok) {
        ...
        const err: any = new Error(`Graph upload failed: ${upRes.status} ${t.slice(0, 200)}`);
      ...
      const err: any = new Error(`Graph PDF convert failed: ${dl.status} ${t.slice(0, 200)}`);
