 ...
 *
 *   1) שתי פסקאות אישור, **מיושרות לימין**:
 *      "בחתימתי מטה אני מאשר כי קראתי את ההצעה, הבנתי את תוכנה ואני מסכים לכל תנאיה."
 *      "ככל שההצעה מאושרת בשם חברה או תאגיד, …"
 *   2) טבלת חתימה בת 4 עמודות (RTL), ממורכזת:
 *      ┌──────────────────────┬──────┬────────┬────────┐
 *      │ שם מלא של מאשר ההצעה  │ ת.ז  │ חתימה  │ חותמת  │
 *      ├──────────────────────┼──────┼────────┼────────┤
 *      │                      │      │        │        │  ← שורה ריקה למילוי ידני
 *      └──────────────────────┴──────┴────────┴────────┘
 *   3) שורת הערה ממורכזת **מתחת** לטבלה (ללא נקודה בסוף):
 *      "במקרה של חברה, יש לצרף חותמת החברה לצד חתימת מורשה החתימה"
 *
 * הפורמט (נוסח, יישור ומבנה הטבלה) הועתק מקובץ ההצעה שהמשתמש אישר כמקור-אמת
 * (א.חן בנגב — הצעת מחיר לראדון).
...
function signatureTableXml(): string {
  ...
  // <w:jc w:val="center"/> ב-trPr (בנוסף לזה שב-tblPr): Word מסתפק ב-tblPr, אבל מנוע
  // ההמרה ל-PDF קורא את המרכוז מכל שורה בנפרד — בלעדיו הטבלה נצמדת לשוליים ב-PDF.
  const headerRow =
    '<w:tr><w:trPr><w:cantSplit/><w:tblHeader/><w:jc w:val="center"/></w:trPr>' +
    headerCell('שם מלא של מאשר ההצעה', wName) +
    headerCell('ת.ז', wId) +
    headerCell('חתימה', wSign) +
    headerCell('חותמת', wStamp) +
    '</w:tr>';
  // שורת מילוי ריקה בגובה ~1.6 ס"מ — באותו סדר עמודות ובאותו מרכוז.
  const fillRow =
    '<w:tr><w:trPr><w:cantSplit/><w:trHeight w:val="900"/><w:jc w:val="center"/></w:trPr>' +
    emptyCell(wName) + emptyCell(wId) + emptyCell(wSign) + emptyCell(wStamp) +
    '</w:tr>';
  return `<w:tbl>${tblPr}${grid}${headerRow}${fillRow}</w:tbl>`;
}

/** עיצוב אחיד לכל טקסט הבלוק — David מודגש 11pt, RTL. */
const BLOCK_RPR =
  '<w:rPr><w:rFonts w:cs="David" w:hint="cs"/><w:b/><w:bCs/><w:sz w:val="22"/><w:szCs w:val="22"/><w:rtl/></w:rPr>';

/**
 * פסקת האישור — מודגשת, David 11pt, RTL, **מיושרת לימין**. שני המשפטים הם *פסקאות
 * נפרדות* (לא שורה אחת עם <w:br/>). קריטי: מעבר-שורה ידני (<w:br/>) בשילוב justify גורם
 * ל-Word למתוח את השורה שלפני ה-break לרוחב מלא → "רווחים ענקיים בין המילים". פסקאות
 * נפרדות פותרות זאת כי Word לא מותח את השורה האחרונה של פסקה.
 *
 * היישור לימין (ולא מרכוז) הועתק מקובץ ההצעה שהמשתמש אישר כמקור-אמת. המשפט השלישי
 * ("במקרה של חברה…") אינו כאן — הוא יורד אל *מתחת* לטבלה, ראו companyStampNote().
 */
function approvalParagraph(): string {
  ...
  // כל משפט = פסקה מיושרת ימין (bidi ללא jc → ימין ב-RTL). keepNext בכולן כדי שהבלוק
  // כולו (פסקאות + טבלה + הערת החותמת) יישאר יחד באותו עמוד.
  const para = (t: string) =>
    '<w:p><w:pPr><w:keepNext/><w:bidi/>' +
    `${BLOCK_RPR}</w:pPr><w:r>${BLOCK_RPR}<w:t xml:space="preserve">${t}</w:t></w:r></w:p>`;
  return sentences.map(para).join('');
}

/**
 * הערת החותמת — שורה בודדת ממורכזת מתחת לטבלה, ללא נקודה בסוף (כמו במקור-האמת).
 * ממוקמת אחרי הטבלה ולפני ה-bookmark הסוגר, כך ש-stripSignatureTable() מסירה אותה יחד
 * עם שאר הבלוק בזרימת החתימה הדיגיטלית.
 */
function companyStampNote(): string {
  const text = 'במקרה של חברה, יש לצרף חותמת החברה לצד חתימת מורשה החתימה';
  return (
    '<w:p><w:pPr><w:keepNext/><w:bidi/><w:jc w:val="center"/>' +
    `${BLOCK_RPR}</w:pPr><w:r>${BLOCK_RPR}<w:t xml:space="preserve">${text}</w:t></w:r></w:p>`
  );
}
...
/**
 * הבלוק המלא: bookmark-פתיחה → רווח → פסקאות אישור → רווח קטן → טבלה → הערת חותמת →
 * bookmark-סגירה.
 * ה-bookmarks הם גבולות בלתי-נראים (לא דולפים ל-PDF). פסקת הרווח שלפני הטבלה נושאת
 * keepNext, ולכן הבלוק כולו (פסקאות + רווח + טבלה + ההערה) נשאר יחד באותו עמוד.
 * הרווח מכוון להישאר קטן — ראו ההערה על GAP_BEFORE_TABLE: בלוק גבוה מדי + keepNext
 * מקפיץ את כל בלוק החתימה לעמוד חדש.
 * הערת החותמת יושבת *בין* הטבלה ל-bookmark הסוגר, ולכן stripSignatureTable() מסירה
 * אותה יחד עם הבלוק בזרימת החתימה הדיגיטלית.
