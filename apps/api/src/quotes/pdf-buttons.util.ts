...
export async function stampQuoteButtons(
  ...
  // כמה מקום אנכי בכלל קיים בעמוד האחרון מעל הרצפה — לא רק מתחת לטקסט. כל עוד יש כאן
  // מקום סביר, מעדיפים *תמיד* להישאר בעמוד האחרון (מוקטנים אם צריך) ולא לקפוץ לעמוד חדש.
  const pageRoom = (ph - bottomMargin) - floorY;
  const MIN_SCALE = 0.35;  // מקטינים עד 35% כדי להישאר בעמוד — עדיף כפתור קטן מכפתור לבד בעמוד חדש.

  if (available >= fullStackH) {
    // יש מספיק מקום מתחת לטקסט — צורבים בגודל מלא מיד מתחת לטקסט.
    topY = textBottom - topGap;
  } else if (available >= fullStackH * MIN_SCALE) {
    // מרווח צר מתחת לטקסט — מקטינים את הכפתור כדי שייכנס בין הטקסט לרצפה.
    scale = available / fullStackH;
    topY = textBottom - topGap;
  } else if (pageRoom >= fullStackH * MIN_SCALE) {
    // אין מספיק מתחת לטקסט (הטקסט יורד נמוך), אבל *יש* מקום בעמוד עצמו: מעגנים את הכפתור
    // לתחתית העמוד (מעל הרצפה/פוטר) ומקטינים אם צריך — במקום לקפוץ לעמוד חדש. הכפתור עשוי
    // לחפוף מעט לתחתית הטקסט, וזה עדיף בהרבה על כפתור בודד בעמוד ריק.
    scale = Math.min(1, pageRoom / fullStackH);
    const stackH = fullStackH * scale;
    topY = floorY + stackH; // הקצה העליון כך שהתחתית יושבת על הרצפה
  } else {
    // באמת אין שום מקום אנכי סביר בעמוד האחרון (פוטר תופס כמעט את כולו) — מוצא אחרון: עמוד חדש.
