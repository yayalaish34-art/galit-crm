...
export class QuoteSignatureService {
   ...
   *  profileUrl — צורב גם את כפתור "לחץ כאן להסמכות שלנו" (קישור לפרופיל+רישיונות). משמש רק
   *  בקבצי ה-PDF שנשלחים בוואטסאפ (מהקונטרולר הציבורי) — לא בזרימת המייל (שם הפרופיל מצורף כקובץ).
   *  הכפתורים ממוקמים מתחת לתוכן (בלי לעלות על טקסט, ולעולם לא לבד בעמוד חדש). ה-PDF השמור נשאר תמיד נקי. */
  async getPdf(
    token: string,
